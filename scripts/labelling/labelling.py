...
from utils.prompts import poor_quality_labelling_prompt, good_quality_labelling_prompt
from utils.pydantic_models import PoorQualityLabellingResponseModel, GoodQualityLabellingResponseModel
...
class Labeller:
    ...
    def label_good_quality_data(self, document_info_df, label_config):
        ...
        for topic in all_topics:
            ...
            user_prompt = user_prompt.replace("`topic`", topic).replace("`label_config`", str(label_config))
        ...
        filtered_doc_info_df.to_csv(path_or_buf=f"data/output/{self.hash_key}/good_quality_data.csv",
                                    index=False)
        save_dict_to_json(data=topic_label_dict, dir_path=f"data/output/{self.hash_key}",
    ...
    def label_poor_quality_data(self, document_info_df, label_config):
        ...
        for i in tqdm(range(0, len(filtered_doc_info_df), iter_size), desc="Labelling Poor Quality Data"):
            chunk = filtered_doc_info_df.iloc[i:i + iter_size]
            topic_dict = {}
            for index, row in chunk.iterrows():
                if not isinstance(row["Document"], float):
                    extracted_keywords = self.keybert_model.extract_keywords(row["Document"],
                                                                             keyphrase_ngram_range=(1, 1),
                                                                             top_n=5)
                    topics = [keyword[0] for keyword in extracted_keywords]
                    topic_dict[row["ID"]] = topics
                    filtered_doc_info_df.loc[
                        filtered_doc_info_df["ID"] == row["ID"], "KeyBERT_Topics"] = str(topics)
        ...
        filtered_doc_info_df.to_csv(path_or_buf=f"data/output/{self.hash_key}/poor_quality_data.csv",
                                    index=False)
    ...
    def collate_labelled_data(self):
        ...
        good_quality_data = pd.read_csv(f"data/output/{self.hash_key}/good_quality_data.csv")
        poor_quality_data = pd.read_csv(f"data/output/{self.hash_key}/poor_quality_data.csv")
        good_quality_data_df = good_quality_data[["Document", "Label"]]
        poor_quality_data_df = poor_quality_data[["Document", "Label"]]
        combined_df = pd.concat([good_quality_data_df, poor_quality_data_df], ignore_index=True)
        ...
        for idx, row in combined_df.iterrows():
            ...
            label = row["Label"]
            train_dataset.loc[
                train_dataset[self.data_col_name].str.contains(re.escape(document), na=False),
                "predicted_label"
            ] = label

        train_dataset.to_csv(path_or_buf=f"data/output/{self.hash_key}/final_labelled_data.csv", index=False)
        accuracy = accuracy_score(train_dataset["label_text"], train_dataset["predicted_label"])
